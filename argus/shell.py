...
def _get_base_directory(directory, image_name):
    """Return the path to the root directory for this run."""
    if directory:
        if not os.path.isdir(directory):
            os.mkdir(directory)
    else:
        directory = tempfile.mkdtemp(prefix="argus-{}-".format(image_name),
                                     dir="/tmp")
...
def main():
    ...
    process = _start_testr(args.parallel,
                           args.tests, base_directory)
